...
public class Player {
	 ...
	 * @param cards The DeckList object containing the list of cards in the player's deck
	 */
	...
	/**
	 * Method to reduce the player's health, called when the enemy attacks the player directly
	 ...
	 * @return The Card object drawn, or null if the draw and discard piles are empty
	 */
	public Card draw() {
		if(drawPile.size() == 0) {
	...
	/**
	 * Method to check if the player's field is empty. Used to determine if enemies attack the player directly or attack cards on the field.
	...
	/**
	 * Disables all player cards to prevent interaction. Stops player from attacking when it's not their turn and from trying to attack themselves.
	 */
	...
	/**
	 * Enables all player cards to allow interaction. Used when it's the player's turn.
	 */
	...
	/**
	 * Method to update the UI to reflect changes in the player's state. Useful for classes that have access to the Player but not the GameGUI.
	 */
	...
	/**
	 * Method to reset the player's mana to the maximum at the start of their turn
	 */
